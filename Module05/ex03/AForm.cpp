...
/*   Updated: 2024/06/20 19:25:07 by blarger          ###   ########.fr       */
/*                                                                            */
/******************************************************************************/
...
AForm::AForm(std::string _name, int _minGradeToSign, int _minGradeToExecute) : name(_name), minGradeToSign(_minGradeToSign), minGradeToExecute(_minGradeToExecute), isSigned(false), signer("N/D")
{
	try
	{
		...
		/* else
			std::cout << GREEN << "AForm constructor called. Setting name to " << this->name << ", minimum grade to sign to " << _minGradeToSign << ", minimum grade to execute " << _minGradeToExecute << RESET << std::endl; */
	}
...
{
	//std::cout << GREEN << "AForm constructor called. Setting name to " << this->name << RESET << std::endl;
}
